...
export const SITE_DESCRIPTION = "Book and manage your bike service appointments online";
...
export const API = {
  ...
  RESERVATIONS: {
    ...
    BY_STATUS: (statuses: string[]) => `/reservations?${statuses.map(status => `status=${status}`).join('&')}`,
  },
  // Service endpoints
  SERVICES: {
    BASE: '/services',
